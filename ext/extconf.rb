...
if `uname -s`.chomp != 'Darwin'
  ...
  core_flags = %W{
    -isysroot /Developer/SDKs/MacOSX#{sdk_verion}.sdk
    -mmacosx-version-min=#{sdk_verion} -mdynamic-no-pic -std=gnu99
  }
  ...
  system "CFLAGS='#{cflags.join(' ')} #{wflags.join(' ')}' /usr/bin/gcc #{gcc_opts.join(' ')} -o '#{gem_root}/bin/fsevent_watch' fsevent/fsevent_watch.c"
